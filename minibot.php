...
$base_h = ["Content-Type: application/json", "X-Telegram-Init-Data: $query", "User-Agent: $ua", "Accept: application/json"];
...
if (isset($login['token'])) {
    ...
    for ($cycle = 1; $cycle <= 3; $cycle++) {
        ...
        echo "[+] Saldo App: " . hijau . $saldo . reset . "\n";
        ...
        if (isset($complete['newBalance'])) {
            echo hijau . "SUKSES! Saldo Sekarang: " . $complete['newBalance'] . reset . "\n";
        } else {
            echo merah . "GAGAL: " . ($complete['error'] ?? 'Sesi bermasalah') . "\n" . reset;
        }
    }
} else {
